...
//           //  className="rounded-full shadow-md w-full h-10 flex justify-center ml-14 items-center bg-blue-500 text-white hover:bg-blue-600 transition duration-200"
//            className="flex items-center ml-16 w-full gap-2"
//           />
//         </div>
...
import { SelectedAccountDisplay } from './SelectedAccountDisplay';

export const BeneficiaryTransfer: React.FC = () => {
  ...
  const openSelector = () => setSelectOpen(true);
  const closeSelector = () => setSelectOpen(false);
  const openConfirm = () => selectedAccount && amount && setConfirmOpen(true);
  ...
  // Find bank logo for selected
  const selectedBankLogo = selectedAccount
    ? banks?.find(b => b.name.toLowerCase() === selectedAccount.bankName.toLowerCase() ||
                       b.slug === selectedAccount.bankName.toLowerCase().replace(/\s+/g, '-'))?.logo
    : undefined;

  return (
    <div className="p-4">
      <h2 className="text-xl font-semibold text-[#343C6A] mb-4">Beneficiary Transfer</h2>
      <div className="flex flex-col gap-4 bg-whit max-w-x w-full rounded-3xl p-4 shadow-md">
        <div className="grid grid-cols-2 gap-4">
          {beneficiaryData.slice(0, 3).map(acc => (
            <BeneficiaryCard
              key={acc.id}
              account={acc}
              bankLogo={banks?.find(b => b.name === acc.bankName)?.logo}
              isSelected={selectedAccount?.accountNumber === acc.accountNumber}
              onSelect={handleAccountSelect}
            />
          ))}
          {/* <Button title="View all" onClick={openSelector} className="flex items-center gap-2 mx-auto p-3  " /> */}
          <button
          onClick={openSelector}
          className="flex-shrink-0 w-12 h-12 rounded-full border-2 border-dashed border-gray-400 flex items-center justify-center"
        >
          +
        </button>
        </div>

        {selectedAccount && selectedBankLogo && (
          <SelectedAccountDisplay account={selectedAccount} bankLogo={selectedBankLogo} />
        )}
        ...
        <div className="flex items-center mt-4">
          <input
            ...
            className="mr-2"
          />
          <label htmlFor="saveDir" className="text-gray-700 text-sm">
            Save to directory of beneficiary
          </label>
        </div>  

        <AmountInput amount={amount} onChange={setAmount} />
        <TransferActions selectedAccount={selectedAccount} amount={amount} onConfirm={openConfirm} />
      </div>

      <DialogBox isOpen={isSelectOpen} onClose={closeSelector} title="Select a Beneficiary">
        <BeneficiarySelector
          searchQuery={''}
          onSearchChange={() => {}}
          onSelect={handleAccountSelect}
